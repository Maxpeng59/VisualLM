(function () {
  ...
  class SandboxRunner {
    ...
    _spawn() {
      ...
      const worker = new Worker("./sandbox-worker.js?v=10");
      this.worker = worker;
  ...
  async function runSceneWithRepair(scene, prompt) {
    ...
    for (let attempt = 0; attempt <= MAX_REPAIRS; attempt++) {
      ...
      try {
        ...
        } else if (current.recovered_after_blank) {
          const n = current.recovered_after_blank;
          setConfidence(
            `${current.dimension} • generated by ${engineLabel}` +
              (current.model ? " (" + current.model + ")" : "") +
              ` — recovered after ${n} blank attempt${n === 1 ? "" : "s"}`,
            "ok",
          );
        } else {
          setConfidence(
